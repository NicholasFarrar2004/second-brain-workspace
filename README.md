...
[Enter the fictional workspace](example/README.md) · [Walk through a task](docs/walkthrough.md) · [Explore the architecture](docs/architecture.md) · [Run the checks](#run-the-checks) · [Design tradeoffs](docs/evolution-and-tradeoffs.md) · [Contribution and provenance](PROVENANCE.md)
...
The architecture separates two decisions: **which files apply** and **what each file owns**. Folder names and [short navigation pages](example/README.md) help a person select the subject. Directory scope answers which instructions apply. Distinct records for rules, durable facts, current state and historical evidence answer what each file owns.
...
The validator audits required files, declared paths and supported local Markdown links across the example, including the new navigation pages. Separately, it reports the selected route's ordered context files and their byte sizes. The structural audit reads other branches too; those files do not enter the selected context list. It does not edit the example, archive it, send prompts or call a model. Byte counts describe these files, not tokens saved or performance gains.
